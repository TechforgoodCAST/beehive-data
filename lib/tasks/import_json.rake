...
namespace :import do
  ...
  task json: :environment do
    ...
    # Regexes
    BEN_REGEXES = {
      :crime => /\b(crim(inal|e)|justice|judicial|offenders?|custody|anti-social behaviour|prison(ers?)?|law centre|victim|murder|rape|theft|fraud)/i, # possible confusion with "social justice" and "restorative justice",
      :relationship => /\b(relationship|marriage|family breakdown|mediation|counselling|conflict resolution)/i, # relationship a bit general
      :disabilities => /\b(disabled|disabilit(ies|y)|blind(ness)?|deaf(ness)?|(hearing|sight) loss|amputee|wheel ?\-?chair|accessib(ility|le)|handicap(ped)?|less abled|sign language|impairment|visual(ly)? ?\-?impair(ment|ed))/i,
      :religion => /\b(christ(ian)?|muslim|jew(ish)|mosque|synagogue|church|buddhis(t|m)|sikh)/i, # bit wide, need to exclude, eg church halls
      :disasters => /\b(flood(s|ing)?|disasters?|rescue|survivors?|tsunami|storms?|hurricane|aftermath)/i, # need a wider range "survivors" has DV use too, rescue used a lot for animals
      :education => /\b(schools?|pupils?|students?|school\-?age|teach(ers?|ing)|college|a ?\-?levels?|g\.?c\.?s\.?e\.?s?|key stage (1|2|3|one|two|three)|mentoring|educational|school(child|boy|girl)(ren|s)?|classroom)/i, # need to exclude pre-school? schools are a venue for lots of activities
      :unemployed => /\b((un)?-?employ(ed|ment)|job-?seekers?|benefit claimants?|claim benefits|jobless|out of work)/i,
      :ethnic => /\b(b\.?a?m\.?e\.?|black|ethnic|racial|roma)/i, # black may be too generic?
      :water => /\b(water|sanitation|toilets?|sewage|hygien(e|ic)|wastewater)/i, # need to exclude water sports
      :food => /\b(food|hunger|feed|local produce|food\-?bank|fruits?|vegetables?|cook(ery|ing)|famines?|(mal)?nutrition(ist)?|meat)/i,
      :housing => /\b(housing|homeless(ness)?|tenants?|rough ?\-?sleep(ing|ers?)|runaways?|residents?|household|shelter)/i, # housing is a verb, and is generic
      :animals => /\b(animals?|pets?|dogs?|cats?|horses?|wildlife|fauna|farm-animal|livestock|marine|habitat|birds?)/i,
      :buildings => /\b((community|new|existing) building|built environment|architecture|refurbish(ing|ment)?|repairs?|restoration|(community|village) hall|Preservation Trust|Building works)/i, # building and heritage both a bit too wide
      :mental => /\b(mental ?\-?(health|illness(es)?|diseases?|disorders?)|depressian|schizophrenia|bi\-?polar|psychiatry|psychiatric|eating ?-?disorders?|Self ?\-?help)/i, # mental disease could cover dementia/learning disabilities
      :orientation => /\b(l\.?g\.?b\.?t?\.?q?\.?|lesbian|gay|bi\-?sexual|sexuality|sexual orientation|trans\-?(sexual|gender)?|homo\-?sexual|queer|cisgender|intersex)/i, # rainbow - but children too
      :environment => /\b(environment(al)?|climat(e|ic)|global warming|carbon|energy efficien(t|cy)|ecosystem|nature|green spaces?|bio\-?diversity|sustainab(ility|le)|countryside|garden|pond|parks?|eco\-?audit|footpaths?|wilderness|greenhouse gas|ecolog(y|ical))/i, # environment a bti broad: learning environment, peaceful environment, etc
      :physical => /\b(physical health|cancer|disease|illness|Down\'?s Syndrome|get fit|fitness|sport(ing|s)?|physiotherapy|Multiple Sclerosis|stroke|diabetes|Healthy Living|health ?care|blood pressure|virus|infection)/i, # exclude mental? often says "physical and mental health"
      :organisation => /\b((this|our) organisation|core (costs|funding))/i, # not sure about this one!
      :organisations => /\b(charities|local groups|community groups|social enterprises|Council for Voluntary Service|VCS)/i,
      :poverty => /\b(poverty|deprivation|isolation|disadvantaged)/i, # "poor" is too generic, used as an adjective
      :refugees => /\b(asylum ?-?seekers?|refugees?|migrants?|displaced persons?)/i, # sanctuary?
      :services => /\b(armed forces|army|navy|air force|marines|armed services|(ex\-)?servicem(e|a)n\'?s?|veterans?|british legion|regiment(al)?|military|sailors?|soldiers?)/i,
      :care => /\b(care ?\-?leavers?|leaving care|looked ?\-?after ?\-?(children)?|carers?|leave care)/i, # definition of care here?
      :exploitation => /\b((sex)? ?\-?traffic(k?ing)?|exploitation|forced labour|sex ?\-?workers?|prostitut(es?|ion))/i,

      # the following are not in the original list
      :abuse => /\b((domestic|sexual) (violence|abuse)|violence against women|honou?r killings?|child abuse)/i,
      :addiction => /\b(addict(ion)?|(alcohol|drug) abuse|alcohol(ism)?|drugs?|narcotics?|abstinence)/i,
      :learningdisabilities => /\b((learning|intellectual) (difficult|disabilit|disorder)(ies|y)?)/i,

      # not really beneficiaries
      #:arts-culture => /\b(arts?|theatre|music(al)?|museums?|galler(y|ies))/i,
      #:sport-recreation => /\b(cricket|rugby|football|Tennis|swimming)/i,
      #:research => /\b()/i,
    }

    GENDER_REGEXES = {
      # the following identify genders
      :men => /\b(m(e|a)n|boys?|fathers?|males?|lads?)/i,
      :women => /\b(wom(e|a)n|girls?|lad(y|ies)|mothers?|females?|lesbians?)/i,
      :transgender => /\b(trans\-?(sexual|gender)?)/i,
    }

    AGE_REGEXES = {
        # the following identify ages or life stages
        :baby => {:regex => /\b(bab(ies|y)|neo\-?nat(al|e)|childbirth|infants?|toddlers?|preschoolers?|newborn)/i, :age_from => 0, :age_to => 3},
        :children => {:regex => /\b(child(ren)|junior|kids)/i, :age_from => 4, :age_to => 11},
        :youngpeople => {:regex => /\b(young ?\-?people|youth|teen\-?agers?|adolescen(ce|ts)|juveniles?|puberty)/i, :age_from => 12, :age_to => 25},
        :adults => {:regex => /\b(adult)/i, :age_from => 25, :age_to => 65},
        :elderly => {:regex => /\b(elderly|old(er)? ?\-?people|pensioners?|senior citizens?|(octo|nona)genarian|((second|2nd|1st|first) world war|WW(II|2)) veteran)/i, :age_from => 65, :age_to => 100},
